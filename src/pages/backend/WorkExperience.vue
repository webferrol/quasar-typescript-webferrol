...
const { setExperiences,deleteExperience } = useStoreExperiences();
const { experiences,getExperiencesLength,loading } = storeToRefs(useStoreExperiences());
...
<template>
    <div class="q-pa-md">
        ...
        <q-table v-else title="Experiencia laboral" :rows="experiences" :rows-per-page-options="[0]" :pagination-label="getPaginationLabel" :columns="columns" row-key="title" selection="multiple"
